...
def build_raw_representations(
    ...
    for index, row in dataframe.iterrows():
        ...
        if (index + 1) % 100 == 0 or (index + 1) == total_samples:
            print(f"Extracted {index + 1}/{total_samples} HOG+LBP raw feature vectors.")
...
def main() -> None:
    ...
    parser.add_argument(
        "--feature-set",
        choices=["hog", "lbp", "hog_lbp", "hog_pca_lbp"],
        default="hog_pca_lbp",
        help="Feature representation: hog, lbp, hog_lbp, or hog_pca_lbp (default).",
    )
    parser.add_argument(
        "--pca-k",
        type=int,
    ...
    if args.feature_set == "hog_pca_lbp":
        print(f"\n--- Extracting raw HOG and LBP representations ---")
        X_hog_tr, X_lbp_tr, y_train = build_raw_representations(train_dataframe)
        X_hog_va, X_lbp_va, y_val = build_raw_representations(val_dataframe)

        print(f"\n--- Fitting PCA (k={args.pca_k}) on Train HOG features ---")
        pca_model = fit_pca(X_hog_tr, n_components=args.pca_k)
        save_pca(pca_model, MODELS_DIR / f"hog_pca_{args.pca_k}.joblib")

        print(f"--- Fusing PCA-reduced HOG + LBP ---")
        X_train = fuse_hog_pca_lbp(X_hog_tr, X_lbp_tr, pca_model)
        X_val = fuse_hog_pca_lbp(X_hog_va, X_lbp_va, pca_model)
    else:
        X_train, y_train = build_feature_matrix(train_dataframe, args.feature_set)
        X_val, y_val = build_feature_matrix(val_dataframe, args.feature_set)
